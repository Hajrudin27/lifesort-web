    ...
    <div>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-stone-900">Opskrifter</h1>
            <p className="text-sm text-stone-500">{totalCount} opskrifter i alt</p>
          </div>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <div className="mt-6 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm shadow-stone-900/5">
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={name} onChange={(e) => setName(e.target.value)}
                className="mt-1 w-full rounded-xl border border-stone-200 px-3 py-2 text-sm outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100"
                placeholder="Fx Kylling i karry" />
            </div>
            <div>
              <label className="text-xs font-semibold text-stone-500">Måltid</label>
          ...
          <div className="mt-4">
            <label className="text-xs font-semibold text-stone-500">Ingredienser</label>
              ...
              {ingredients.map((ing, i) => (
                <div key={i} className="flex gap-2">
                  <input value={ing.name} onChange={(e) => updateIngredient(i, 'name', e.target.value)}
                    ...
                    className="flex-1 rounded-xl border border-stone-200 px-3 py-2 text-sm outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100" />
                  <input value={ing.amount} onChange={(e) => updateIngredient(i, 'amount', e.target.value)}
                    placeholder="Mængde (fx 400 g)"
                    className="w-40 rounded-xl border border-stone-200 px-3 py-2 text-sm outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100" />
                  {ingredients.length > 1 && (
                    <button onClick={() => removeIngredientRow(i)}
                      className="rounded-xl border border-stone-200 px-3 text-red-500 transition hover:bg-red-50">
                      <X size={14} />
                    </button>
                  )}
                </div>
            ...
            <button onClick={addIngredientRow} className="mt-2 flex items-center gap-1 text-sm font-medium text-amber-600 hover:underline">
              <Plus size={14} /> Tilføj ingrediens
            </button>
          </div>

          <div className="mt-4">
            <label className="text-xs font-semibold text-stone-500">Fremgangsmåde</label>
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-stone-500">Tid (minutter)</label>
              <input type="number" value={minutes} onChange={(e) => setMinutes(e.target.value)}
                className="mt-1 w-full rounded-xl border border-stone-200 px-3 py-2 text-sm outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100"
                placeholder="30" />
            </div>
            <div>
              <label className="text-xs font-semibold text-stone-500">Tags (kommasepareret)</label>
          ...
          <div className="mt-4">
            <label className="text-xs font-semibold text-stone-500">Næringsindhold (valgfrit, per portion)</label>
            <div className="mt-1 grid grid-cols-4 gap-3">
              <input type="number" value={calories} onChange={(e) => setCalories(e.target.value)} placeholder="Kalorier"
                className="rounded-xl border border-stone-200 px-3 py-2 text-sm outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100" />
          ...
          <div className="mt-6 flex gap-2">
            ...
            <button onClick={resetForm} className="rounded-xl border border-stone-200 px-4 py-2 text-sm font-medium text-stone-600 hover:bg-stone-50">
              Annullér
            </button>
          </div>
        </div>
        ...
        <div className="mt-6 flex gap-3">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Søg opskriftsnavn..."
              className="w-full rounded-xl border border-stone-200 bg-white py-2 pl-10 pr-3 text-sm outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100" />
          </div>
          <select value={mealFilter} onChange={(e) => setMealFilter(e.target.value as MealType | 'all')}
            className="rounded-xl border border-stone-200 bg-white px-3 py-2 text-sm outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100">
        ...
        <div className="mt-4 overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm shadow-stone-900/5">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-stone-100">
              {isLoading ? (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-stone-400">Indlæser...</td></tr>
              ) : rows.length === 0 ? (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-stone-400">Ingen opskrifter fundet.</td></tr>
              ) : (
                rows.map((row) => (
                  <tr key={row.id} className="transition hover:bg-stone-50/50">
                    ...
                    <td className="px-5 py-3.5 text-stone-500">{row.ingredients.length} ingredienser</td>

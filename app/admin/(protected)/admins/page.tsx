  ...
  return (
    <div>
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-stone-900">Admins</h1>
          <p className="text-sm text-stone-500">{adminRows?.length ?? 0} med adgang til panelet</p>
        </div>
      </div>

      <InviteAdminForm />

      <div className="mt-6 overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm shadow-stone-900/5">
            ...
            {(adminRows ?? []).map((row) => (
              <tr key={row.id} className="transition hover:bg-stone-50/50">
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-3">
                    <span className="flex h-8 w-8 items-center justify-center rounded-full bg-rose-500/10 text-xs font-bold text-rose-600">
                      {initials(row.full_name)}
                    </span>
                    <span className="font-medium text-stone-900">{row.full_name}</span>
                  </div>
                </td>
                <td className="px-5 py-3.5 text-stone-600">{emailById.get(row.id) ?? '—'}</td>
                <td className="px-5 py-3.5">
